            ...
            <div class="col-md-12">
                ...
                <div class="card mb-2">
                    ...
                    <div class="card-body px-2 pt-0 pb-2 p-4">
                        <form action="{{ route('articles.update', ['article' => $posts->id]) }}" class="px-4"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="title" id="title"
                                    placeholder="Input title" value="{{ $posts->title }}" disabled>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="category_id" id="category_id" class="form-control" disabled>
                                    @foreach ($categories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $posts->category_id == $item->id ? 'selected' : '' }}>{{ $item->category }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="penulis" id="penulis"
                                    placeholder="Input penulis" value="{{ $posts->penulis }}" disabled>
                            </div>
                            <div class="form-group mb-4">
                                <label for="isi">Isi</label>
                                <textarea name="isi" id="isi" class="form-control" rows="20" cols="10" disabled>{!! $posts->body !!}</textarea>
                            </div>
                            <div class="row mb-2">
                                <div class="col-md-3">
                                    <img id="previewImg" alt="PreviewImg"
                                        src="{{ asset('storage/articles') }}/{{ $posts->gambar }}" height="200"
